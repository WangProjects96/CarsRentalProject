...
public class FenetreLocAuto extends JFrame {
	// jdialog
	private String nomFenetre = "Louer une voiture";
	...
	private ArrayList<Exemplaire> ex = Exemplaires.getVoitureDispo();
	...
	public void lancer() {
		...
		for (Exemplaire exemplaire : ex) {
			voitures.addItem(exemplaire.toString());
		}
		...
		voitures.setBounds(100, 75, 250, 25);
			...
			if (event.getSource() == boutonLouer) {
				try {
					...
					loc = new Location(dateDebut, dateFin, emp.get(users.getSelectedIndex()),
							ex.get(voitures.getSelectedIndex()), assu);
