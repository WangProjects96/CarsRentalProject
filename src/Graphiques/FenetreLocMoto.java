...
public class FenetreLocMoto extends JFrame {
	private String nomFenetre = "Louer une moto";
	...
	private ArrayList<Exemplaire> ex = Exemplaires.getMotoDispo();
	...
	public void lancer() {
		...
		for (Exemplaire exemplaire : ex) {
			motos.addItem(exemplaire.toString());
		}
			...
			if (event.getSource() == boutonLouer) {
				try {
					...
					loc = new Location(dateDebut, dateFin, emp.get(users.getSelectedIndex()),
							ex.get(motos.getSelectedIndex()), assu);
					fermer();
				} catch (Exception e) {
